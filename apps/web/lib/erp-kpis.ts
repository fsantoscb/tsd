...
export async function erpKpis(grain:KpiGrain,from:string,to:string,shift="ALL"){
  ...
  const[allEvents,allLabour,rates,resources]=await Promise.all([
    ...
    paged((start:number,end:number)=>d.from("v_current_labour_segments").select("person_key,area_code,operational_date,shift_code,hour_bucket,paid_hours,regular_hours,overtime_hours,paid_break_hours,productive_hours,approval_status").eq("organization_id",c.organizationId).gte("operational_date",from).lte("operational_date",to).order("segment_start").range(start,end)),
  ...
  for(const x of[rates,resources])if(x.error)throw x.error;const events=shift==="ALL"?allEvents:allEvents.filter((x:any)=>x.shift_code===shift),labour=shift==="ALL"?allLabour:allLabour.filter((x:any)=>x.shift_code===shift);
