...
export async function ingestUpDailyActuals(value:any){const organizationId=String(value?.organizationId??""),from=String(value?.from??""),to=String(value?.to??""),rows=Array.isArray(value?.rows)?value.rows:[];if(!/^[0-9a-f-]{36}$/i.test(organizationId)||!/^\d{4}-\d{2}-\d{2}$/.test(from)||!/^\d{4}-\d{2}-\d{2}$/.test(to)||from>to||rows.length>1000)throw new Error("INVALID_UP_DAILY_PAYLOAD");await assertConfiguredOrganization(organizationId);const{data,error}=await admin().rpc("replace_up_daily_actuals",{p_organization_id:organizationId,p_from:from,p_to:to,p_rows:rows});if(error)throw new Error(error.message);return{accepted:Number(data??0)}}
export async function ingestDtgOrderHistory(value:any){const organizationId=String(value?.organizationId??""),orderNos=Array.isArray(value?.orderNos)?value.orderNos.map(String):[],rows=Array.isArray(value?.rows)?value.rows:[];if(!/^[0-9a-f-]{36}$/i.test(organizationId)||orderNos.length>5000||rows.length!==orderNos.length)throw new Error("INVALID_DTG_ORDER_HISTORY_PAYLOAD");await assertConfiguredOrganization(organizationId);const allowed=new Set(orderNos),records=rows.map((row:any)=>{const orderNo=String(row?.orderNo??"");if(!orderNo||!allowed.has(orderNo))throw new Error("INVALID_DTG_ORDER_HISTORY_ORDER");return{organization_id:organizationId,order_no:orderNo,printed_garments:Number(row.printedGarments??0),printed_prints:Number(row.printedPrints??0),first_pick:row.firstPick??null,first_print:row.firstPrint??null,last_print:row.lastPrint??null,source_max_audit_id:row.sourceMaxAuditId??null,refreshed_at:new Date().toISOString()}});if(!records.length)return{accepted:0};const{error}=await admin().from("dtg_order_history_summaries").upsert(records,{onConflict:"organization_id,order_no"});if(error)throw new Error(error.message);return{accepted:records.length}}
