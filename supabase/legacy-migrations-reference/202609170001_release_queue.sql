alter table source_orders add column if not exists route_id text;
alter table source_orders add column if not exists cost_centre text;
alter table source_orders add column if not exists stop_ship_flag text;
alter table sync_batches add column if not exists release_line_count integer not null default 0;
...
create table if not exists source_release_order_lines(
 id bigint generated always as identity primary key,organization_id uuid not null,sync_batch_id uuid not null references sync_batches(id) on delete cascade,
 order_no text not null,line_number text not null,product text,client text,qty_lcd numeric not null default 0,orig_ref3 text,group_code text,product_name text,source_updated_at timestamptz,created_at timestamptz not null default now(),
 unique(sync_batch_id,order_no,line_number)
);
create index if not exists source_release_lines_order_idx on source_release_order_lines(organization_id,order_no);
alter table source_release_order_lines enable row level security;
revoke all on source_release_order_lines from anon,authenticated;

create or replace view v_current_release_order_lines as
select l.* from source_release_order_lines l join v_latest_completed_batch b on b.id=l.sync_batch_id;

create or replace view v_release_queue_all as
with process_qty as(
 select organization_id,sync_batch_id,order_no,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code)) in('DTG_1','DTG_2')),0) dtg_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='UNDERPRINT'),0) underprint_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='UV PRINT'),0) uv_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='HATS'),0) hats_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='FINISHED'),0) finished_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='STICKERS'),0) stickers_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='VISUAL'),0) visual_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='PROD'),0) production_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='CUSTOM EMB'),0) custom_emb_qty,
  coalesce(sum(qty_lcd) filter(where upper(trim(group_code))='EYEWEAR'),0) eyewear_qty
 ...
 where o.site='B' and o.order_no like '13%' and o.source_status='1' and o.date_due between now()-interval '30 days' and now()+interval '30 days'
), resolved as(
 ...
 array_remove(array[case when upper(trim(route_id))='NO' then 'ROUTE_BLOCKED' end,case when upper(trim(stop_ship_flag))='Y' then 'STOP_SHIP' end,case when custom_emb_qty>0 then 'CUSTOM_EMB' end],null)::text[] release_blockers
 from evidence e
)
select r.*,case when total_process_qty<=0 then 'ZERO_PRODUCTION_QTY' end diagnostic_status,
 case when route_id is null or trim(route_id)='' or stop_ship_flag is null or trim(stop_ship_flag)='' or date_due is null or source_status is null then 'UNKNOWN'
 when upper(trim(coalesce(cost_centre,'')))='NOTAPPRO' then 'NOT_APPROVED'
 when cardinality(release_blockers)>0 then 'BLOCKED'
 when date_due::date>((now() at time zone 'Australia/Brisbane')::date+7) then 'FUTURE_DUE'
 else 'ELIGIBLE' end release_status
from resolved r;

create or replace view v_release_queue as select * from v_release_queue_all where total_process_qty>0;
grant select on v_current_release_order_lines,v_release_queue_all,v_release_queue to authenticated,service_role;
...
begin
 ...
 insert into sync_batches(organization_id,status,connector_version)values(v_organization_id,'running',payload->>'connectorVersion')returning id into batch_id;
 begin
  truncate table source_release_order_lines,source_orders,source_workbank_items,source_stock_items;
  insert into source_orders(organization_id,sync_batch_id,order_no,date_received,date_due,date_released,source_status,source_sub_status,customer_code,customer_name,ship_to_name,customer_state,city,delivery_desc,client_so_number,source_priority,site,route_id,cost_centre,stop_ship_flag,source_updated_at)
  select v_organization_id,batch_id,x."orderNo",x."dateReceived",x."dateDue",x."dateReleased",x."sourceStatus",x."sourceSubStatus",x."customerCode",x."customerName",x."shipToName",x."customerState",x.city,x."deliveryDesc",x."clientSoNumber",x."sourcePriority",x.site,x."routeId",x."costCentre",x."stopShipFlag",x."sourceUpdatedAt" from jsonb_to_recordset(payload->'orders')x("orderNo" text,"dateReceived" timestamptz,"dateDue" timestamptz,"dateReleased" timestamptz,"sourceStatus" text,"sourceSubStatus" text,"customerCode" text,"customerName" text,"shipToName" text,"customerState" text,city text,"deliveryDesc" text,"clientSoNumber" text,"sourcePriority" integer,site text,"routeId" text,"costCentre" text,"stopShipFlag" text,"sourceUpdatedAt" timestamptz);
  insert into source_release_order_lines(organization_id,sync_batch_id,order_no,line_number,product,client,qty_lcd,orig_ref3,group_code,product_name,source_updated_at)
  select v_organization_id,batch_id,x."orderNo",x."lineNumber",x.product,x.client,x."qtyLcd",x."origRef3",x."groupCode",x."productName",x."sourceUpdatedAt" from jsonb_to_recordset(coalesce(payload->'releaseOrderLines','[]'::jsonb))x("orderNo" text,"lineNumber" text,product text,client text,"qtyLcd" numeric,"origRef3" text,"groupCode" text,"productName" text,"sourceUpdatedAt" timestamptz);
 ...
 exception when others then update sync_batches set status='failed',error_message=left(sqlerrm,1000)where id=batch_id;raise;end;return batch_id;
end $$;
alter function ingest_sync_batch(jsonb) set statement_timeout='120s';
revoke all on function ingest_sync_batch(jsonb) from public,anon,authenticated;grant execute on function ingest_sync_batch(jsonb) to service_role;
