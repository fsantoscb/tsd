...
with stock as (
  select s.organization_id,regexp_replace(s.product,'^#','') order_no,count(*) box_count,
    sum(s.production_units) remaining_units,min(s.source_timestamp) last_movement
  from v_current_stock s
  where upper(coalesce(s.location,''))='UNDERPRINT'
  group by s.organization_id,regexp_replace(s.product,'^#','')
), putwall as (
  select organization_id,order_no,
    string_agg(distinct coalesce(nullif(to_location,''),from_location),', ' order by coalesce(nullif(to_location,''),from_location)) putwall_locations
  from v_current_workbank where upper(coalesce(from_zone,''))='PWL1'
...
select w.organization_id,w.order_no,max(coalesce(w.customer_name,o.customer_name)) customer_name,max(o.delivery_desc) screen,
  max(o.source_status) status,max(coalesce(w.source_priority,o.source_priority)) priority,min(coalesce(w.source_due_at,o.date_due)) date_due,
  greatest(0,current_date-coalesce(min(coalesce(w.source_due_at,o.date_due))::date,current_date)) age_days,
  count(*) item_count,count(*)::numeric remaining_units,null::timestamptz last_movement,
...
where upper(coalesce(w.from_zone,''))='DTGS'
