...
describe("routing master rules", () => {
  ...
  it("keeps the source revision input unchanged", () => { const source=[{revision:1},{revision:2}];nextRoutingRevision(source);expect(source).toEqual([{revision:1},{revision:2}]); });
});
