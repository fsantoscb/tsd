alter table public.production_daily_actuals
  add column if not exists shift_code text not null default 'OUT_OF_SHIFT'
  check (shift_code in ('SHIFT_1','SHIFT_2','SHIFT_3','OUT_OF_SHIFT'));

alter table public.production_daily_actuals
  drop constraint if exists production_daily_actuals_organization_id_operational_date_process_machine_code_key;
