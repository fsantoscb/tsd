import {readFile,writeFile} from "node:fs/promises";import {parseConnectorEnv} from "./env";import {testOracleConnection} from "./oracle";import {OracleSourceReader} from "./source-reader";import {syncOnce} from "./sync";import{agentTick,heartbeatOnce}from"./agent";
...
async function main(){const configured=parseConnectorEnv(process.env);const env={...configured,AUDIT_AFTER_ID:await readAuditCursor(configured.AUDIT_AFTER_ID)};const command=process.argv[2];
 ...
 throw new Error("Expected oracle:test, sync:once, agent:heartbeat or agent:tick");
}
