import {describe,expect,it} from "vitest";import {mapAudit,mapReleaseOrderLine,mapStock,mapWorkbank} from "../src/mapping";
describe("Oracle mapping",()=>{
 ...
 it("preserves the raw authoritative release value without inference",()=>{expect(mapReleaseOrderLine({ORDER_NO:"1301",LINE_NUMBER:2,QTY_LCD:10,RELEASED:"Y"}).released).toBe("Y");expect(mapReleaseOrderLine({ORDER_NO:"1301",LINE_NUMBER:3,QTY_LCD:10,RELEASED:null}).released).toBeNull()});
});
